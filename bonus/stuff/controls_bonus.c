...
/*   Updated: 2025/04/16 11:15:15 by achbira          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	use_legs(t_vect vec, int neo, t_player *p, t_data *all)
{
	...
	if (y > 0 && y < all->coords.height && (all->map[(int)y][(int)p->x] != '1' &&  all->map[(int)y][(int)p->x] != '2'))
		p->y = p->y + neo * vec.y * all->move.mov_sped;
	if (x > 0 && x < all->coords.width && (all->map[(int)p->y][(int)x] != '1' &&  all->map[(int)p->y][(int)x] != '2'))
